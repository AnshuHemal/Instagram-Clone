...
export default function ClosedCaptionsScreen() {
  ...
  const handleSwitchChange = (val: boolean) => {
    ...
    showToast({ 
      message: val ? 'Closed captions enabled' : 'Closed captions disabled', 
      type: 'info' 
    });
  };

  const handleDisplayLanguagePress = () => {
    haptics.light();
    showToast({ message: 'Closed captions language options coming soon!', type: 'info' });
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { paddingTop: insets.top + 6, borderBottomColor: isDark ? '#262626' : '#DBDBDB' }]}>
        ...
        <Text style={[styles.headerTitle, { color: isDark ? '#FFFFFF' : '#000000' }]}>Closed captions</Text>
      </View>

      <ScrollView contentContainerStyle={styles.scrollContent}>
        {/* Turn on closed captions Row */}
        <Animated.View 
        ...
        <Animated.View entering={FadeInDown.delay(180).duration(350)}>
          <Pressable 
            onPress={handleDisplayLanguagePress}
            style={({ pressed }) => [
              styles.navRow,
          ...
          >
            ...
            <View style={styles.rightContainer}>
              <Text style={[styles.valueText, { color: textGray }]}>English</Text>
