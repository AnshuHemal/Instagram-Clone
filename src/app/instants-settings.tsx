...
export default function InstantsSettingsScreen() {
  const { colors, isDark } = useTheme();
  const { showToast } = useToast();
  const router = useRouter();
  const insets = useSafeAreaInsets();

  const [instants, setInstants] = useState<'off' | 'on'>('on');
  const [emojiReactions, setEmojiReactions] = useState<'off' | 'on'>('on');
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        <Animated.View entering={FadeInDown.duration(300)}>
          {renderRadioSection('Instants', instants, setInstants, 'janeappleseed shared an instant.')}
          {renderRadioSection('Emoji reactions', emojiReactions, setEmojiReactions, 'janeappleseed reacted 😂 to your instant.')}
